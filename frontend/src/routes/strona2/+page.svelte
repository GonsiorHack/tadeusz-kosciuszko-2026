<script>
  ...
  let autoProgressTimeout = undefined;
  
  const messages = [
    "Strona 2 - Historia się rozwija...",
    "Tutaj możesz dodać kolejną część historii",
    "Zmień te wiadomości na swoje!",
    "To jest przykład czwartej wiadomości",
    "I piąta wiadomość na tej stronie"
  ];
  ...
  function handleClick() {
    ...
    // If typing, complete the message immediately
    if (isTyping) {
      clearInterval(typeInterval);
    ...
    if (currentMessageIndex < messages.length - 1) {
      ...
      sessionStorage.setItem('strona2_index', currentMessageIndex.toString());
      // Clear strona1 position since we're moving forward
      sessionStorage.setItem('strona1_index', (messages.length - 1).toString());
      typeMessage();
    } else {
      // Save last position when leaving
      sessionStorage.setItem('strona2_index', (messages.length - 1).toString());
      location.href = '/strona3';
    }
  }
  
  // Type first message on mount
  onMount(() => {
    // Check if returning from next page
    const savedIndex = sessionStorage.getItem('strona2_index');
    if (savedIndex) {
      currentMessageIndex = parseInt(savedIndex);
    } else {
      // Clear the index for strona3 when starting fresh
      sessionStorage.removeItem('strona3_index');
    }
...
<main>
    <nav class="glass-nav">
        <div class="nav-container">
            <button class="back-button" onclick={() => (location.href = '/strona1')}>
            ...
            <div class="page-counter">2 / 10</div>
        </div>
    </nav>
    ...
    <section class="story-section">
        ...
        <div class="story-content king-right">
            <div class="chat-bubble-container" 
                ...
                <p class="hint-text">
                    {currentMessageIndex < messages.length - 1 ? 'Kliknij, aby kontynuować →' : 'Kliknij, aby przejść dalej →'}
                </p>
            </div>
