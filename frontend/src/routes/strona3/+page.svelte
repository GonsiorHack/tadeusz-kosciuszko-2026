<script>
  ...
  const messages = [
    ...
    "Czwarta wiadomość na stronie 3",
    "I kolejna przed przejściem dalej"
  ];
      ...
      if (charIndex < fullText.length) {
        ...
        autoProgressTimeout = setTimeout(() => {
          if (currentMessageIndex < messages.length - 1) {
            currentMessageIndex++;
            sessionStorage.setItem('strona3_index', currentMessageIndex.toString());
            typeMessage();
          } else {
            sessionStorage.setItem('strona3_index', currentMessageIndex.toString());
            sessionStorage.setItem('strona2_index', (messages.length - 1).toString());
            location.href = '/strona4';
          }
        }, 3000);
      }
    }, 50);
  ...
  function handleClick() {
    if (isTyping) {
      // If currently typing, complete the message immediately
      if (typeInterval) {
        clearInterval(typeInterval);
      }
      ...
      // If not typing, proceed to next message immediately
      if (autoProgressTimeout) {
        clearTimeout(autoProgressTimeout);
      }
      if (currentMessageIndex < messages.length - 1) {
  ...
  onMount(() => {
    const savedIndex = sessionStorage.getItem('strona3_index');
    if (savedIndex) {
...
<main>
    ...
    <section class="story-section">
        ...
        <div class="story-content king-center">
            <div class="king-container king-centered">
                <img class="story-king" src="/src/lib/assets/whiteKing.svg" alt="White King" />
                <div class="king-glow"></div>
            </div>
            
            <div class="chat-bubble-container bubble-below" 
                 onclick={handleClick} 
